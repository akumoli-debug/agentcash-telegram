...
| --- | --- | --- |
| Telegram private wallet | stable MVP | `/start`, `/deposit`, `/balance`, `/cap`, `/policy`, `/spend`, `/research`, `/search`, `/exa`, `/enrich`, `/generate`, `/history`, `/freeze`, `/unfreeze`, and `/status` are implemented and covered by automated tests around wallet provisioning, quote records, policy decisions, spend analytics, confirmations, history, freeze behavior, privacy guards, and replay rejection. Private-wallet commands are private-chat only. |
