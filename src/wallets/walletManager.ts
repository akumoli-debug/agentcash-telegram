...
export class WalletManager {
  ...
  async getOrCreateWalletForTelegramUser(
    ...
    const homeDirHash = WalletManager.getHashedHomeDirName(
      telegramId,
    ...
    wallet = this.db.updateWallet(wallet.id, {
      ...
      encryptedPrivateKey:
        ensuredWallet.encryptedPrivateKey ?? wallet.encrypted_private_key,
      status: "active"
    });
  ...
  async getBalance(
    ...
    WalletContextResult & {
      balance: Awaited<ReturnType<AgentCashClient["getBalance"]>>;
    }
  > {
    const { user, wallet } = await this.getOrCreateWalletForTelegramUser(
      telegramId,
      profile
    );
  ...
  async getDepositAddress(
    ...
    WalletContextResult & {
      deposit: Awaited<ReturnType<AgentCashClient["getDepositInfo"]>>;
    }
  > {
    const { user, wallet } = await this.getOrCreateWalletForTelegramUser(
      telegramId,
